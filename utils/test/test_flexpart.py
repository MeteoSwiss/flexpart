...
def mock_s3_endpoint(aws_credentials):
    ...
    with mock.patch.dict(os.environ, {"AWS_ENDPOINT_URL": s3_endpoint}):
...
def mock_environment(monkeypatch):
    ...
    monkeypatch.setenv("FORECAST_DATETIME", '20241210000000')
...
def test_flexpart_run(mock_s3_endpoint, mock_environment):
    ...
    process = subprocess.run(["bash", entrypoint], capture_output=True)
    ...
    for path in path_list:
        key = f"{md.model}/{md.date}_{md.time}/{os.getenv('RELEASE_SITE_NAME')}/{md.number:03}/{path.name}"
